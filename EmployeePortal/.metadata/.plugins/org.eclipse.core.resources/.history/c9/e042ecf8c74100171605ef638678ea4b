	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		    ...
		    if(empid1!=null)
			{
				...
				/*	String special="";
					...
					try 
					{
						PreparedStatement preparedStatement=connection.prepareStatement("Update EmployeeDetails set name=?,address=?,company=?,age=?,gender=?,salary=? ,specialization=?, language=? where empid=?");
						preparedStatement.setString(1, Name);
						preparedStatement.setString(2, Address);
						preparedStatement.setString(3, Company);
						preparedStatement.setString(4, Age);
						preparedStatement.setString(5, Gender);
						preparedStatement.setString(7, special);
						preparedStatement.setString(8, lang);
		//				preparedStatement.setInt(8, sessionid);
						preparedStatement.setString(6, Salary);
						preparedStatement.setString(9, empid1);
